...
CI checks correctness; use the constrained benchmark runner for resource measurements.

## Output and numerical behavior
...
- Structured output prints Python byte-string representations for sample tags.
  ...
  See `scripts/compare_outputs.py` for text-output comparisons.
...
- The differential-test quantifier restores requested coverage/sample order
  ...
  `scripts/compare_quantify.py` covers reversed groups, filter_idx, fractional
...
- Normalization `uq` raises NameError because count.py does not import
  scoreatpercentile. Native code reports this error.
- Correction `TSBH` passes `tsbh` to statsmodels 0.14.4, which rejects that
  ...
  by division by zero. Native code reports these errors.
...
- SciPy 1.13.1's `xlog1py` calls its renamed Cephes `log1p`, despite the
  Cython source importing `libc.math.log1p`. Ruspladder uses the same
  approximation from unity.c for likelihood and dispersion calculations.
...
- JSON comparison probes enable exact float round trips. The default parser
  can change individual inputs by one ULP, affecting ill-conditioned
  dispersion calculations. HDF5 production values do not
  pass through JSON.
- Test input processing retains the source's two different outlier caps
  ...
  ties-to-even count rounding, design columns and dPSI mask.
  `scripts/compare_test_input.py` compares inputs at the GLM boundary.
- Test output retains NumPy's unstable floating-point argsort for equal
  ...
  values. `scripts/compare_test_output.py` checks the writer with fixed
  statistical results; `scripts/compare_test_cli.py` checks the full command.
- The real gene-expression column selection and quantified PSI are Fortran-order
  ...
  use pairwise addition. Input comparisons preserve this array order.
  CLI checks include non-alt normalization,
...
- The committed mutex fixture has too few distinct raw dispersions after
  ...
  fixture in `scripts/compare_test_cli_synthetic.py` exercises all six event
  types through the full command.

The CLI test path explicitly disables exon-count augmentation of isoform counts
and event-ID construction.
...
201 distinct introns in `scripts/compare_build_sequence.py` changes the current
exon minimum from 17 to 21, while retention stays at 17. Native options retain
these distinct filter states across sequential builds.
...
forwards `--reference` to HTSlib. `scripts/compare_prep_cli.py` compares arrays against the upstream
`summarize_chr` API supplied with its expected attribute.
This comparison covers the API rather than the broken upstream CRAM prep CLI.
...
introns to another gene or failing out of bounds. Ruspladder preserves these
results and reports the corresponding errors. `scripts/compare_sparse_build.py`
covers these cases. Normal `merge_graphs` builds each sample before merging.
