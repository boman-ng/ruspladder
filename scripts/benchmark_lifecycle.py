...
def main():
 ...
 a=p.parse_args();task_root=Path(os.environ.get('RUSPLADDER_WORK_ROOT','/home/wubw/data/ruspladder'));a.work.mkdir(parents=True,exist_ok=True)
 before=snapshot();assert len(before['cpu_affinity'])==8 and before['effective_memory_limit']==16*1024**3
 if a.dataset=='airway':
 ...
 build=[str(binary),'build','-a',str(local_annotation),'-b',','.join(map(str,paths)),'-o',str(a.work/'results'),'--parallel','8',*extra]
 if reference:build+=['--reference',str(reference)]
 if a.sparse:build+=['--sparse-bam']
 test=[str(binary),'test','-o',str(a.work/'results'),'-a',','.join(names[:split]),'-b',','.join(names[split:]),'--parallel','8','--event-types','exon_skip','--dpsi','0']
 ...
 report=dict(mode=a.mode,dataset=a.dataset,cache=a.cache,sparse=a.sparse,through=a.through,binary_sha256=binary_hash,wall_seconds=elapsed,cpu_seconds=cpu,mean_cores=cpu/elapsed,utilization_8cpu_percent=12.5*cpu/elapsed,max_process_rss_kib=usage.ru_maxrss,cgroup_peak_bytes=after['memory_limits'][0]['peak'],input_blocks=usage.ru_inblock-usage_before.ru_inblock,output_blocks=usage.ru_oublock-usage_before.ru_oublock,stages=stages,resources_before=before,resources_after=after,returncode=returncode,soft_timeout_seconds=1800,hard_timeout_seconds=3600,soft_exceeded=soft_exceeded or elapsed>=1800,timed_out=timed_out)
