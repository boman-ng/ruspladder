...
def main():
 ...
 before=snapshot();assert len(before['cpu_affinity'])==4 and before['effective_memory_limit']==8*1024**3
 if a.dataset=='airway':
 ...
 usage_before=resource.getrusage(resource.RUSAGE_CHILDREN);start=time.monotonic();stages=[];returncode=0
 for name,command in ([('build',build)] if a.through=='build' else [('build',build),('test',test)]):
  ...
  with (a.work/f'{a.cache}-{name}.log').open('w') as log:process=subprocess.run(command,stdout=log,stderr=subprocess.STDOUT)
 ...
 report=dict(mode=a.mode,dataset=a.dataset,cache=a.cache,sparse=a.sparse,through=a.through,binary_sha256=binary_hash,wall_seconds=elapsed,cpu_seconds=cpu,mean_cores=cpu/elapsed,utilization_4cpu_percent=25*cpu/elapsed,max_process_rss_kib=usage.ru_maxrss,cgroup_peak_bytes=after['memory_limits'][0]['peak'],input_blocks=usage.ru_inblock-usage_before.ru_inblock,output_blocks=usage.ru_oublock-usage_before.ru_oublock,stages=stages,resources_before=before,resources_after=after,returncode=returncode)
