...
def main():
    ...
    manifest_path = Path(__file__).resolve().parent.parent / "fixtures/upstream-manifest.json"
    manifest = json.loads(manifest_path.read_text())
    verify_files(args.upstream, manifest)
    ...
    if sha256(args.binary) != binary_hash:
        raise ValueError("Runtime binary changed during comparisons")
    report = dict(reference_commit=commit, binary_sha256=binary_hash, threads=4, cases=reports, result="pass")
